...
def colorize_image(target_image, reference_images):
    ...
    inlier_threshold = 8.0  # 调整 inlier_threshold 的值
    # inlier_threshold = 50.0
    ...
    for i in range(len(reference_images)):
        if inliers[i] and homographies[i] is not None:  # 確認 homographies[i] 不為 None
            # 對參考圖像應用投影變換
            projected_image = cv2.warpPerspective(reference_images[i], homographies[i], (target_image.shape[1], target_image.shape[0]))

            # 計算光照（使用最大值運算）
            weight = 0.15 # 调整权重以平衡光照估计和反射估计
            ...
            # mse = np.square(projected_image.astype(np.float64) - target_image.astype(np.float64))
            # reflectance += mse
...
# 載入目標圖像和參考圖像
target_image = cv2.imread("newtarget_gray(7).jpg")
reference_folder = "input2"
...
# 保存結果
cv2.imwrite('colorized_image_finalForInput2.jpg', colorized_image)
# cv2.imshow('colorized_image__finalForInput2.jpg', colorized_image)
# cv2.waitKey(0)
